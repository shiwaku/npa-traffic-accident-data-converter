...
with open(output_csvfile, 'a', encoding='UTF-8') as f:
    ...
    for i in range(len(data)):
        ...
        csvfile_writer.writerow({
            ...
            '負傷者数': data[i][6], '路線名': res_rosenkoudo, '上下線': res_jyougesen, '地点コード': data[i][9], '市区町村コード': data[i][10],
            ...
            '環状交差点の直径': res_kanjyoukousatennotyokkei, '信号機': res_singouki, '一時停止規制_標識（当事者A）': res_itijiteisikisei_hyousiki_a, '一時停止規制_表示（当事者A）': res_itijiteisikisei_hyouji_a, '一時停止規制_標識（当事者B）': res_itijiteisikisei_hyousiki_b,
            ...
            '曜日(発生年月日)': res_youbi, '祝日(発生年月日)': res_syukujitu, '地点_緯度（北緯）_10進数': data[i][58], '地点_経度（東経）_10進数': data[i][59]
        })
